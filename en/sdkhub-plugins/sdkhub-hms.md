...
Obtains a token that a target app developer applies for a sender to integrate Push Kit in the multi-sender scenario. The **subjectId** parameter needs to be set to the sender project ID in AGC console.

The usage and callback are the same as the `startPush` method.

**Parameter Description**:

| Parameter name | Fill format | Description |
| :--- | :--- | :--- |
| subjectId | "7364300792444xxxxx" | Project ID of the sender. Sign in to AGC console, select **My projects**, find your project in the project list, and go to **Project settings**. On the page that is displayed, obtain the project ID of the sender. |
...
Deletes a token that a target app developer applies for a sender to integrate Push Kit in the multi-sender scenario. The subjectId parameter needs to be set to the sender project ID in AGC console.

The usage and callback are the same as the `closePush` method.
...
Refer to [Account Verification](https://developer.huawei.com/consumer/en/doc/development/HMSCore-Guides/account-profile-0000001057562781). From Push SDK 5.0.4, Push Kit provides the account verification function for you to push messages based on accounts on devices. For example, users A and B use the same Huawei mobile phone, and have HUAWEI Wallet. The messages pushed by HUAWEI Wallet to user A are displayed only when user A signs in.
...
| :--- | :--- | :--- |
| subjectId | "7364300792444xxxxx" | Project ID of the sender. Sign in to AGC console, select **My projects**, find your project in the project list, and go to **Project settings**. On the page that is displayed, obtain the project ID of the sender. |
| type | "HUAWEI_PROFILE" | Type of the account to be verified. The options are as follows: `HUAWEI_PROFILE`: HUAWEI ID; `CUSTOM_PROFILE`: other.  |
| profileId | "A123456" | If the device supports the account verification function, the function will be enabled when an app pushes messages. You can generate a profileId parameter as the unique identifier of an account. |
